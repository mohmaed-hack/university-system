...
function SubjectDetailModal({
  ...
  const lectures = lectureType === 'theory' ? subject.lecturesTheory : subject.lecturesPractical
...
function LecturesPage({
  ...
  return (
    <>
      <Dialog open={isOpen && !selectedLecture} onOpenChange={(o) => { if (!o) onClose() }}>
        <DialogContent className="max-w-md w-full p-0 overflow-hidden rounded-2xl gap-0">
                ...
                {lectures.map((lecture, idx) => (
                  <motion.button
                    ...
                    className="group flex items-center gap-4 p-4 rounded-xl bg-card border border-border
                      hover:border-primary/30 hover:shadow-md transition-all duration-200 text-right"
                  >
                    <div className={cn(
                      'flex items-center justify-center size-10 rounded-xl font-black text-sm',
                      isTheory
  ...
  return (
    <>
      <Dialog open={!showVideoMessage} onOpenChange={(o) => { if (!o) onClose() }}>
        <DialogContent className="max-w-sm w-full p-6 rounded-2xl">
          <DialogTitle className="sr-only">{lecture.title}</DialogTitle>

          {/* Header */}
          <div className="flex items-center gap-3 mb-4">
            <div className={cn(
              'flex items-center justify-center size-12 rounded-xl font-black',
              isTheory
                ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400'
                : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'
            )}>
            ...
            <div className="flex flex-col gap-0.5">
              <h3 className="font-bold text-foreground">{lecture.title}</h3>
              {lecture.date && (
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 w-full py-3 rounded-xl
                bg-red-500/10 text-red-600 dark:text-red-400 font-bold text-sm
                border border-red-400/20 hover:bg-red-500/20 transition-all duration-200"
            >
              <Play className="size-4" />
              {lecture.videoUrl ? 'مشاهدة الفيديو' : 'الفيديو'}
            </button>
          </div>
        </DialogContent>
      </Dialog>
